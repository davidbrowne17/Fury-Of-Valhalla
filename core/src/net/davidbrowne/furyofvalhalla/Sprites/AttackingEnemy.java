...
public class AttackingEnemy extends Enemy {
    ...
    public AttackingEnemy(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        this.screen = screen;
        frames = new Array<TextureRegion>();
        for(int i=0; i<3;i++){
            frames.add(screen.getAtlas().findRegion("saxon"));
            frames.add(screen.getAtlas().findRegion("saxon_walk_1"));
            frames.add(screen.getAtlas().findRegion("saxon_walk_2"));
        }
        walkAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        for(int i=0; i<3;i++){
            frames.add(screen.getAtlas().findRegion("saxon_attack1"));
            frames.add(screen.getAtlas().findRegion("saxon_attack2"));
            frames.add(screen.getAtlas().findRegion("saxon_attack3"));
            frames.add(screen.getAtlas().findRegion("saxon_attack4"));
        }
        attackAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        frames.add(screen.getAtlas().findRegion("saxon_dead"));
        frames.add(screen.getAtlas().findRegion("saxon_dead_1"));
        frames.add(screen.getAtlas().findRegion("saxon_dead_2"));
