...
public class Jotun extends Enemy {
    ...
    public Jotun(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        this.screen = screen;
        frames = new Array<TextureRegion>();
            for(int i=0; i<3;i++){
                frames.add(screen.getAtlas().findRegion("jotun"));
                frames.add(screen.getAtlas().findRegion("jotun_walk_1"));
                frames.add(screen.getAtlas().findRegion("jotun_walk_2"));
            }
        walkAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        for(int i=0; i<3;i++){
            frames.add(screen.getAtlas().findRegion("jotun_attack1"));
            frames.add(screen.getAtlas().findRegion("jotun_attack2"));
            frames.add(screen.getAtlas().findRegion("jotun_attack3"));
            frames.add(screen.getAtlas().findRegion("jotun_attack4"));
        }
        attackAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        frames.add(screen.getAtlas().findRegion("jotun_dead"));
        frames.add(screen.getAtlas().findRegion("jotun_dead_1"));
        frames.add(screen.getAtlas().findRegion("jotun_dead_2"));

        deadAnimation = new Animation(.2f,frames);
        stateTime= 0;
        setBounds(getX(),getY(),20/ Game.PPM,20/Game.PPM);
        setToDestroy=false;
        destroyed=false;
    ...
    protected void defineEnemy() {
        ...
        vertice[0] = new Vector2(-9,9).scl(1/Game.PPM);
        vertice[1] = new Vector2(9,9).scl(1/Game.PPM);
        vertice[2] = new Vector2(-9,-9).scl(1/Game.PPM);
        vertice[3] = new Vector2(9,-9).scl(1/Game.PPM);
        head.set(vertice);
        fdef.shape = head;
        head.dispose();
        fdef.density=0;
