...
public class Nord extends Enemy {
    ...
    public Nord(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        this.screen = screen;
        frames = new Array<TextureRegion>();
        for(int i=0; i<3;i++){
            frames.add(screen.getAtlas().findRegion("nord"));
            frames.add(screen.getAtlas().findRegion("nord_walk_1"));
            frames.add(screen.getAtlas().findRegion("nord_walk_2"));
        }
        walkAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        for(int i=0; i<3;i++){
            frames.add(screen.getAtlas().findRegion("nord_attack1"));
            frames.add(screen.getAtlas().findRegion("nord_attack2"));
            frames.add(screen.getAtlas().findRegion("nord_attack3"));
            frames.add(screen.getAtlas().findRegion("nord_attack4"));
        }
        attackAnimation = new Animation(.2f,frames);
        frames = new Array<TextureRegion>();
        frames.add(screen.getAtlas().findRegion("nord_dead"));
        frames.add(screen.getAtlas().findRegion("nord_dead_1"));
        frames.add(screen.getAtlas().findRegion("nord_dead_2"));
